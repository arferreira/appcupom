module CuponsHelper
  ...
  def publish_cupon cupon
   if false && (session[:facebook_user]) && current_user.privacy_ids.include?(1)
      ...
      options = { :message => "bleh", :description => "blah", :link => url, :picture => "http://dev.nowon.com.br/assets/mobile/homescreen-logo.png" }
      options = {
        :name     => "#{offer.resume} em #{offer.company_name}",
        :description => "Descontos exclusivos para usar agora",
        :link        => url,
        :picture     => "http://dev.nowon.com.br/assets/mobile/homescreen-logo.png"
      }
      graph.put_connections("me", "#{FacebookAPI.app_namespace}:buy", :offer => url)
      graph.put_wall_post("Acabei de comprar uma oferta no Nowon!", options)
    end
  end
