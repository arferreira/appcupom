...
class CuponsController < ApplicationController
  ...
  def confirm_with_credit_only
    ...
    if @current_user.total_credit_value >= @offer.nowon_price
      @cupon = Cupon.create  :user_id => @current_user.id,
                             :offer_id => @offer.id,
                             :price => @offer.nowon_price,
                             :credit_discount => @offer.nowon_price,
                             :good_date => Time.now,
                             :cupon_code => "",
                             :monthly_cupon_accounting_id => 0,
                             :transaction_id => @transaction_id,
    ...
    respond_to do |format|
      format.html { redirect_to :controller => "cupons", :action => "index", :user_id => current_user.id}
    end
  end
  ...
  def receive_key
    ...
    unless cupon.nil?
      ...
      user_cards = cupon.user.user_cards
      unless user_cards.empty? && !user_cards[0].save_card
    ...
    respond_to do |format|
      format.html { render 'cupons/moip_ok', :status => 200 }
    end
  end
